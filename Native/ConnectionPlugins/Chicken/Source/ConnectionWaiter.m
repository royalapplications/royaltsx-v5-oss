...
- (void)lookupFailed: (NSNumber *)error
{
    ...
    NSString *actionStr = ChickenVncFrameworkLocalizedString(@"NoNamedServer", nil);
    NSString *message;
...
- (void)connectionFailed: (NSString *)cause
{
    ...
    actionStr = ChickenVncFrameworkLocalizedString(@"NoConnection", nil);
    ...
    cause = ChickenVncFrameworkLocalizedString(cause, nil);
    [self error:actionStr message:cause];
}

- (void)serverClosed
{
    NSString    *templ = ChickenVncFrameworkLocalizedString(@"NoConnection", nil);
    NSString    *actionStr;

    actionStr = [NSString stringWithFormat:templ, host, port];
    [self error:actionStr message:ChickenVncFrameworkLocalizedString(@"ServerClosed", nil)];
}
...
- (void)error:(NSString*)theAction message:(NSString*)message
{
    ...
    if ([delegate respondsToSelector:@selector(connectionFailedWithError:andMessage:)])
        [delegate connectionFailedWithError:theAction andMessage:message];
