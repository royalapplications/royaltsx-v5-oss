...
+(NSString*)stringForKeyEvent:(NSEvent*)theEvent ignoreModifiers:(BOOL)ignoreModifiers forLanguage:(NSString*)languageName
{
    CGKeyCode keyCode = theEvent.keyCode;
    NSUInteger modifierFlags = ignoreModifiers ? 0 : theEvent.modifierFlags;
    
    if (keyCode < 1 ||
        keyCode > 47) {
        if (ignoreModifiers) {
            return theEvent.charactersIgnoringModifiers;
        } else {
            return theEvent.characters;
        }
    }
    
    NSDictionary* dict = [NSDictionary dictionaryWithObject:languageName forKey:(NSString*)kTISPropertyLocalizedName];
    NSArray* array = [(NSArray*)TISCreateInputSourceList((CFDictionaryRef)dict, false) autorelease];
    TISInputSourceRef currentKeyboard = (TISInputSourceRef)[array objectAtIndex:0];
    CFDataRef layoutData = TISGetInputSourceProperty(currentKeyboard, kTISPropertyUnicodeKeyLayoutData);
    const UCKeyboardLayout *keyboardLayout = (const UCKeyboardLayout *)CFDataGetBytePtr(layoutData);
    int kbdType = keyboardLayout->keyboardTypeList[0].keyboardTypeFirst;
...
- (void)queueKeyEventFromEvent: (NSEvent *)theEvent
{
    ...
    //NSString *unmodified = [EventFilter stringForKeyEvent:theEvent ignoreModifiers:YES forLanguage:@"U.S."];
    //NSString *modified = [EventFilter stringForKeyEvent:theEvent ignoreModifiers:NO forLanguage:@"U.S."];
...
{
	[self _updateCapsLockStateIfNecessary];
	int index, strLength = (int)[string length];
