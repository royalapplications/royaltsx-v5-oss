...
- (void)setUncompressedData:(NSData *)nsData
{
	...
	for(tile.origin.y = frame.origin.y; tile.origin.y < frame.origin.y+frame.size.height; tile.origin.y += TILE_HEIGHT) {
		...
		for(tile.origin.x = frame.origin.x; tile.origin.x < frame.origin.x+frame.size.width; tile.origin.x += TILE_WIDTH) {
            ...
            if (--length <= 0) {
                [connection terminateConnection:ChickenVncFrameworkLocalizedString(@"ZrleTooSmall", nil)];
                return;
            }
			subEncoding = *data++;
//			NSLog(@"Subencoding = %d\n", subEncoding);
			if(subEncoding == 0) {
				...
				while(y < (tile.size.width * tile.size.height)) {
                    if (length < cPixelSize) {
                        [connection terminateConnection:ChickenVncFrameworkLocalizedString(@"ZrleTooSmall",
                                                                          nil)];
                        return;
                    }
					...
					do {
                        if (--length < 0) {
                            [connection terminateConnection:
                                ChickenVncFrameworkLocalizedString(@"ZrleTooSmall", nil)];
                            return;
                        }
						b = *data++;
						i += b;
                ...
                format = ChickenVncFrameworkLocalizedString(@"ZrleUnknownSubencoding", nil);
