...
- (id)initWithConnection:(RFBConnection*)aConnection andDelegate:(id)aDelegate andMainWindow:(NSWindow*)aWindow {
...
- (IBAction)makeConnectionFullscreen: (id)sender {
	...
	if (displayFullscreenWarning) {
        ...
        NSMenuItem              *menuItem;
        NSString *header = ChickenVncFrameworkLocalizedString( @"FullscreenHeader", nil );

        [[connection eventFilter] synthesizeRemainingEvents];

        [reason appendString: ChickenVncFrameworkLocalizedString( @"FullscreenReason1", nil )];
        ...
        if (scen && menuItem) {
            ...
            if (keyStr) {
                ...
                [reason appendString: @") "];
                [reason appendString: ChickenVncFrameworkLocalizedString(@"FullscreenReason2", nil)];
            } else {
                reason = [NSMutableString stringWithString:ChickenVncFrameworkLocalizedString(@"FullscreenNoKey", nil)];
                header = ChickenVncFrameworkLocalizedString(@"FullscreenNoKeyHeader", nil);
            }
        } else {
            [reason appendString: ChickenVncFrameworkLocalizedString(@"FullscreenReason2", nil)];
        }
        ...
        [alert addButtonWithTitle:ChickenVncFrameworkLocalizedString(@"Fullscreen", nil)];
        [alert addButtonWithTitle:ChickenVncFrameworkLocalizedString(@"Cancel", nil)];
