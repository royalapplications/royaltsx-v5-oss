...
- (void)setServerVersion:(NSData*)aVersion
{
	...
	if (sscanf(cStr, rfbProtocolVersionFormat, &serverMajorVersion, &serverMinorVersion) < 2) {
        [session terminateConnection: ChickenVncFrameworkLocalizedString(@"NotVNC", nil)];
        return;
    }
...
- (void)authenticationFailedFx:(NSString *)reason
{
    [session authenticationFailedFx:reason];
}
...
- (void)readData:(NSNotification*)aNotification
{
    ...
    do {
        ...
        if(length <= 0) {	// server closed socket
            NSString *reason = ChickenVncFrameworkLocalizedString( @"ServerClosed", nil );
        ...
        while(length) {
            ...
            if (consumed == 0) {
                [self terminateConnection: ChickenVncFrameworkLocalizedString(@"ProtocolError", nil)];
                [pool release];
                return;
            }
...
- (void)sendPasteboardToServer:(NSPasteboard *)pb automaticallyAllowingLossyConversion:(BOOL)automaticallyAllowLossyConversion
{
	NSString    *str = [pb stringForType:NSPasteboardTypeString];
    const char  *cStr = [str cStringUsingEncoding:NSISOLatin1StringEncoding];

    if (cStr == NULL) {
        if (automaticallyAllowLossyConversion) {
            ...
            NSBeginAlertSheet(ChickenVncFrameworkLocalizedString(@"PasteConversionHeader", nil),
                              ChickenVncFrameworkLocalizedString(@"PasteAnyways", nil),
                              ChickenVncFrameworkLocalizedString(@"Cancel", nil), nil,
                              [rfbView window], self,
                              ...
                              ChickenVncFrameworkLocalizedString(@"PasteConversionBody", nil));
        }
    } else {
...
- (void)reallyWriteBytes:(unsigned char*)bytes length:(unsigned int)length
{
    ...
    do {
        ...
        if(result >= 0) {
			...
			NSString *reason = ChickenVncFrameworkLocalizedString( @"ServerError", nil );
...
- (NSString *)infoString
{
    ...
    endian = ChickenVncFrameworkLocalizedString([frameBuffer serverIsBigEndian] ?  @"big-endian" : @"little-endian", nil);
    trueColor = ChickenVncFrameworkLocalizedString(frameBuffer->pixelFormat.trueColour ? @"yes" : @"no", nil);

    return
        [NSString stringWithFormat: ChickenVncFrameworkLocalizedString(@"ServerInfo", nil),
...
- (NSString *)statisticsString
{
    ...
    return
        [NSString stringWithFormat: ChickenVncFrameworkLocalizedString(@"ConnectionStatistics", nil),
...
- (void)installMouseMovedTrackingRect
{
	...
	//if (mouseInVisibleRect) // TODO: FX EDIT
