...
- (void)firstTimeConnecting:(NSString *)fingerprint
{
    ...
    NSString    *msg = ChickenVncFrameworkLocalizedString(@"FirstTimeMessage", nil);

    if ([delegate respondsToSelector:@selector(connectionPrepareForSheet)])
        [delegate connectionPrepareForSheet];

    [alert setMessageText:ChickenVncFrameworkLocalizedString(@"FirstTimeHeader", nil)];
    [alert setInformativeText:[NSString stringWithFormat:msg, fingerprint]];
    [alert addButtonWithTitle:ChickenVncFrameworkLocalizedString(@"Connect", nil)];
...
- (void)tunnelEstablishedAtPort:(in_port_t)aPort
{
    ...
    if ((currentSock = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
        NSString *fmt = ChickenVncFrameworkLocalizedString(@"TunnelSocketErr", nil);
    ...
    if (connect(currentSock, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
        NSString *fmt = ChickenVncFrameworkLocalizedString(@"TunnelConnectErr", nil);
        [self tunnelledConnFailed:[NSString stringWithFormat:fmt, strerror(errno)]];
        errno = 0;
        return;
    }
...
- (void)tunnelledConnFailed: (NSString *)err
{
    NSString *fmt = ChickenVncFrameworkLocalizedString(@"TunnelNoConnection", nil);
...
{
    [self tunnelledConnFailed:ChickenVncFrameworkLocalizedString(@"ServerClosed", nil)];
    [tunnel close];
}

- (void)sshFailedWithError:(NSString *)err
{
    NSString    *fmt = ChickenVncFrameworkLocalizedString(@"SshError", nil);
